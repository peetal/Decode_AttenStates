...
"""This script test the model performance on epochs of the left-out-subject"""
##
...
#             fp.write(str(confidence) + '\n')
        ...
        df = pd.DataFrame({"sub_id": np.repeat(int(left_out_subj), 32).tolist(), 
                            "acc": np.repeat((num_correct / epochs_per_subj), 32).tolist(), 
                            "conf": clf.decision_function().tolist(),
                            "cond": np.repeat(str(cond), 32).tolist(),
                            "idx": np.repeat(str(idx), 32).tolist()})
